turtles-own [
  ...
  alpha               ; cost of quality
  beta                ; the sensitivity of the quality
  Aa
  profit
  demand
  ;theta               ; a fraction of consumers in the market, denotes that the variety seeking consumers
]
...
globals [
  consumers-p1           ; The patches that will act as consumers, either a vertical line or all patches
  consumers-p2 
  vs-people-p1
  vs-people-p2
  theta
...
to setup-consumers
  ...
  set consumers-p1 ifelse-value (layout = "line")
    [ patches with [ pxcor = -5 ]]
    [ patches ]
    
  set consumers-p2 ifelse-value (layout = "line")
    [ patches with [ pxcor = 5 ]]
    [ patches ]  
...
to setup-stores 
    ...
    ask patch -5 50 [
      sprout 1 [
        set color red ; use the color from the list that we are looping through
        set shape "Circle (2)" 
        set size 2 
        set Aa 8
        set h 5
    ...
    ask patch -5 -50 [
      sprout 1 [
        set color blue ; use the color from the list that we are looping through
        set shape "Circle (2)" 
        set size 2 
        set Aa 0
        set h 5
...
to-report getService-one 
  let currentService ((1 - theta) * beta * price / (2 * h * alpha))

  report currentService
end

to-report getService-two 
  let currentService ((1 + theta) * beta * price / (2 * h * alpha))
...
to go
  ...
  ;foreach service-changes run
  ;recalculate-area-with-vs
  setup-prices
  ;period-zero
  period-one
  period-two
...
to default-changes
  set theta theta + 0.01 
end

to setup-prices
  
  ask turtle 0 [
    set price h + ((h * alpha * a) / (3 * h * alpha - beta * beta * theta * theta - beta * beta)) 
  ]
...
to calculate-area
  ...
  set vs-people-p1 n-of (theta * 100) consumers-p1
  ;set vs-people-p2 n-of (theta * 40) consumers-p2  
  ...
  ask consumers-p2 [ 
    set preferred-store choose-store2 
    set pcolor ([ color ] of preferred-store + 2)
  ]
...
to-report choose-store 
  ...
  report max-one-of (turtle-set turtle 0 turtle 1) [ V - (price) + beta * (service) - (distance myself / 101) * h + Aa]

end

to-report choose-store2 
  
  report max-one-of (turtle-set turtle 2 turtle 3) [ V - (price) + beta * (service) - (distance myself / 101) * h + Aa]
...
to service-changes
  ...
  ;let currentS service

    ask turtle 2 [
      set service getService-two
    ]
...
to recalculate-area-with-vs
  ; Have each consumer (patch) indicate its preference by 
  ; taking on the color of the store it chooses

  let pys [pycor] of vs-people-p1
  ...
  [ 
    ...
    [
      ...
      set preferred-store choose-store-with-vs
      set pcolor ([color] of preferred-store - 2)
    ]
  ] 
...
to-report choose-store-with-vs
  
report max-one-of turtles [ V - (price) + beta * (service) - (distance myself / 101) * h + Aa - level]
...
to calculate-demand
   ...
   let sa1 [service] of turtle 0
   let sb1 [service] of turtle 1
   let sa2 [service] of turtle 2
   let sb2 [service] of turtle 3
   let pa [service] of turtle 2
   let pb [service] of turtle 3
   
   ask turtle 2 
   [
      set demand (((1 + theta) * theta * (sa2 - sb2)) / (2 * h)) + ((pb - pa + a + h) / (2 * h)) - (theta * beta * (sa1 - sb1) / (2 * h))  
   ]
   
   ask turtle 3
   [
      set demand (((1 + theta) * theta * (sb2 - sa2)) / (2 * h)) + ((pa - pb - a + h) / (2 * h)) - (theta * beta * (sb1 - sa1) / (2 * h))  
   ]
...
to calculate-profit
   ...
   ask turtle 2
   [
     set profit price * demand - (alpha * service * service) / 2   
   ]
...
to-report new-location-task
  ...
  let possible-moves shuffle sort (neighbors4 with [ member? self consumers-p1 ])
...
to-report potential-cost-with-vs
...
to-report new-service-task
  ...
  let possible-service fput service shuffle list (service - 1) (service + 1)

  ; pair each potential price change with its potential revenue
  ; and sort them in decreasing order of revenue
  let service-with-revenues
    sort-by [ last ?1 > last ?2 ]
    map [ list ? (potential-revenue ?) ] possible-service

  let all-zeros? (not member? false map [ last ? = 0 ] service-with-revenues)
  let chosen-service ifelse-value (all-zeros? and service > 1)
    [ service - 1 ] ; if all potential revenues are zero, the store lowers its price as an emergency procedure if it can
    [ first first service-with-revenues ] ; in any other case, we pick the price with the best potential revenues

  let store self ; put self in a local variable so that it can be "captured" by the task
  report task [
    ask store [
      set service chosen-service
    ]
  ]
end
...
to-report potential-revenue [ target-service ]
  let current-service service
  set service target-service
  let new-revenue (potential-market-share * target-service)
  set service current-service
  report new-revenue
end


to-report potential-revenue-with-vs [ target-service ]
  let current-service service
  set service target-service
  let new-revenue (potential-market-share * target-service)
  set service current-service
...
true
false
"ask turtles [\n  create-temporary-plot-pen (word who)\n  set-plot-pen-color color\n]" "plot ([profit] of turtle 2 - [profit] of turtle 3)"
PENS
