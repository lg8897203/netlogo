...
to setup-consumers
  ...
  set consumers ifelse-value (layout = "line")
    [ patches with [ pxcor = 0 ] with [ pycor != 0 ]]
    [ patches ]
  set theta 0.25
  set ppp n-of (theta * 40) consumers
 
end

to setup-consumers-2nd
  ; Store the agentset of patches that are going to be our
  ; consumers in a global variable for easy reference.
  set consumers ifelse-value (layout = "line")
    [ patches with [ pxcor = 10 ] with [ pycor != 0 ]]
    [ patches ]
  set theta 0.25
  set ppp n-of (theta * 40) consumers
...
to setup-stores 
    ...
    ask patch 0 20 [
      sprout 1 [
        ...
        set h 5
        set alpha 1
        set service (getService-one price)
...
to-report getService-one [targetprice]
  let currentService ((1 - theta) * beta * price / (2 * h * alpha))
  show currentService
  report currentService
end

to-report getService-two [s]
  let currentService s + (price * theta * beta) / (h * alpha)
  show "current S"
  show currentService
  report currentService
end
...
to go
  ...
  let service-changes ifelse-value (rules = "moving-only")
    [ (list) ] ; if we are doing "moving-only", the list of price changes is empty
    [ [ new-service-task-with-vs ] of turtles ]
...
to calculate-area
  ...
  ask ppp [ 
    set level 5
    set utility-a 20 - distance turtle 0 
    set utility-b 20 - distance turtle 1
    set pcolor ([color] of preferred-store - 2)
  ]
...
to-report getUtility []
  let currentU 20 - price + beta * service - (distance turtle 0 / 40) * h
  
   
end

to-report getUtility-with-vs [u1]
...
to-report new-service-task-with-vs
  ...
  report task [
    ask store [
      set service getService-two currentS
    ]
  ]
end
...
to-report choose-store
  ...
  report max-one-of turtles [ (service) - (distance myself) ]
end

to-report choose-store-with-vs
  ;show utility-a    
  ifelse utility-a > utility-b [
  let utility-a-2 (utility-a - level)
  let utility-b-2 (utility-b + level)
  set utility-a utility-a-2
  set utility-b utility-b-2
  ;show "t0"
  ;show utility-a
  ;show utility-b
  report turtle 0    
